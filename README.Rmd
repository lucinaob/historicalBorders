```         
---
title: "historicalBorders"
author: "Lucy O'Brien and Lily Gould"
date: "2025-11-18"
output: 
  html_document:
    keep_md: true
---
```

Use: This package allows the user to choose a continent, region, or country and the year that they want to visualize, and a static map producing the location's borders (lasting 10+ years) will be shown at this time is generated.

**Installation**

Installing historicalBorders can be done through the use of

```{r}
install.packages("historicalBorders")
```

*Development Version* \# 
#
install.packages("remotes") remotes::install_github("r-historicalBorders/historicalBorders")

# install.packages("pak")

pak::pak("r-historicalBorders/historicalBorders")

# Or from r-universe

install.packages("historicalBorders", repos = c("<https://r-tmap.r-universe.dev>", "<https://cloud.r-project.org>"))

Details:

Users have dates to choose from spanning to back to 1800 and incremented by every 50 years. Users can examine different scales, including country vs region vs world.

The borders closest to the year inputted by the user will be returned (ex: if a user inputted 1847, it would return a map of borders in 1850).
